...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($address_err) && empty($city_err) && empty($postal_code_err) && empty($phone_err)){
        ...
        $sql = "UPDATE person SET name=?, address=?, city=?, postal_code=?, phone=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_address, $param_city, $param_postal_code, $param_phone, $param_id);
